...
bool TensorDumper::IsDumpFullContent() const { return fullContent_; }
...
bool TensorDumper::DumpTensorHashValue(const std::vector<uint8_t>& hostData, std::string& fileName)
{
    if (csvFile_ == nullptr)
    {
        ...
        if (!Utility::FileCreateManager::GetInstance(GetConfig().outputDir)
                 .CreateCsvFile(&csvFile_, devId, WATCH_CSV_FILE_PREFIX, WATCH_DUMP_DIR, WATCH_HASH_HEADERS))
        {
...
std::string TensorDumper::GetFileName(const std::string& op, std::string watchedOpName, uint64_t index,
                                      bool isWatchStart)
{
