...
namespace MemScope
{
PTAWorkspacePoolTrace::PTAWorkspacePoolTrace() { ptaWorkspaceDomain_ = new mstxDomainRegistration_st{}; }
...
void PTAWorkspacePoolTrace::Reallocate(mstxDomainHandle_t domain, mstxMemRegionsRegisterBatch_t const *desc)
{
    ...
    for (size_t i = 0; i < desc->regionCount; i++)
    {
        ...
        if (!EventReport::Instance(MemScopeCommType::SHARED_MEMORY)
                 .ReportMemPoolRecord(EventSubType::PTA_WORKSPACE, usage, std::move(stack)))
        {
